...
public class ScrapTask {
    ...
    @Scheduled(cron = "0 */6 * * *", initialDelay = 300)
    public void getDateFromSite() throws IOException, InterruptedException {
        ...
        for (String curBaseUrl : baseUrls) {
            ...
            for (Node subject : subjectNodes) {
                ...
                if (!arxivSubject1.isBlank()) {
                    if (!subjectRepository.existsSubjectByTitle(arxivSubject1)) {
                        Subject newSubject = new Subject();
                        newSubject.title(arxivSubject1);
                        subjectRepository.save(newSubject);
                        subjectHashSet.add(newSubject);
                    } else {
                        subjectHashSet.add(subjectRepository.findSubjectByTitle(arxivSubject1));
                    }
                }
            }
